...
import edu.hei.school.agricultural.controller.mapper.CollectivityTransactionDtoMapper;
...
public class CollectivityController {
    ...
    private final CollectivityTransactionDtoMapper collectivityTransactionDtoMapper;
    ...
    public ResponseEntity<?> createCollectivity(@Valid @RequestBody List<CreateCollectivity> createCollectivities) {
        try {
            if (createCollectivities == null || createCollectivities.isEmpty()) {
                return ResponseEntity.status(BAD_REQUEST)
                        .body("Collectivities list cannot be null or empty");
            }
    ...
    public ResponseEntity<?> createCollectivityMembershipFee(
        ...
        try {
            if (membershipFees == null || membershipFees.isEmpty()) {
                return ResponseEntity.status(BAD_REQUEST)
                        .body("Membership fees list cannot be null or empty");
            }
    ...
    public ResponseEntity<?> getCollectivityFinancialAccounts(
        ...
        try {
            return ResponseEntity.status(OK)
                    .body(collectivityService.getFinancialAccounts(id, at).stream()
    ...
    public ResponseEntity<?> getCollectivityTransactions(
        ...
        try {
            return ResponseEntity.status(OK)
                    .body(collectivityService.getTransactions(id, from, to).stream()
                            .map(collectivityTransactionDtoMapper::mapToDto)
                            .toList());
        } catch (NotFoundException e) {
            return ResponseEntity.status(NOT_FOUND)
                    .body(e.getMessage());
