        ...
        try {
            if (createMemberDtos == null || createMemberDtos.isEmpty()) {
                return ResponseEntity.status(BAD_REQUEST)
                        .body("Members list cannot be null or empty");
            }
    ...
    public ResponseEntity<?> createMemberPayments(
        ...
        try {
            if (createMemberPayments == null || createMemberPayments.isEmpty()) {
                return ResponseEntity.status(BAD_REQUEST)
                        .body("Member payments list cannot be null or empty");
            }
            return ResponseEntity.status(HttpStatus.CREATED)
                    .body(memberService.createMemberPayments(id, createMemberPayments).stream()
